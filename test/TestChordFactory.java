...
public class TestChordFactory {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Chord 钢琴C = ChordFactory.majorTriad(rootNoteC, length);
            钢琴C.setDurationFromPace(pace);
            player.playChord(钢琴C);
            钢琴C.showDebugInfo();
            System.out.println("  |> 播放: C Major (默认乐器: 钢琴)");
            Thread.sleep(1500);

            Chord 钢弦吉他C = ChordFactory.majorTriad(rootNoteC, length);
            钢弦吉他C.setDurationFromPace(pace);
            player.playChord(钢弦吉他C);
            钢弦吉他C.showDebugInfo();
            System.out.println("  |> 播放: C Major (指定乐器: 钢弦吉他)");
            Thread.sleep(1500);

            Chord 小号C = ChordFactory.majorTriad(rootNoteC, length);
            小号C.setDurationFromPace(pace);
            ...
            Chord 钢琴CMinor = ChordFactory.minorTriad(rootNoteC, length);
            钢琴CMinor.setDurationFromPace(pace);
            player.playChord(钢琴CMinor);
            钢琴CMinor.showDebugInfo();
            System.out.println("  |> 播放: C Minor (默认乐器: 钢琴)");
            Thread.sleep(1500);

            Chord 小提琴CMinor = ChordFactory.minorTriad(rootNoteC, length);
            小提琴CMinor.setDurationFromPace(pace);
            player.playChord(小提琴CMinor);
            System.out.println("  |> 播放: C Minor (指定乐器: 小提琴)");
            Thread.sleep(1500);

            Chord CMinor = ChordFactory.minorTriad(rootNoteC, length);
            CMinor.setDurationFromPace(pace);
            player.playChord(CMinor);
            System.out.println("  |> 播放: C Minor (指定乐器: 单簧管, 音量: 80 - 柔和)");
            Thread.sleep(1500);

            // --- 3. 测试属七和弦 (Dominant 7th) ---
            System.out.println("\n--- 测试 Dominant 7th Chords ---");
            Chord Dominant7th = ChordFactory.dominant7th(rootNoteG, length);
            Dominant7th.setDurationFromPace(pace);
            player.playChord(Dominant7th);
            System.out.println("  |> 播放: G7 (默认乐器: 钢琴)");
            Thread.sleep(1500);

            // player.playChord(ChordFactory.dominant7th(rootNoteG, length));
